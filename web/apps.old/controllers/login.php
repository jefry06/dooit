...
class login extends CI_Controller {
	...
	function __construct(){
		...
		$this->session->set_userdata(array('company' => $companyData));
	}
	...
	public function authenticate(){			
		...
		if($process){		
			...
			if($result->responseCode == 200){
				...
				/* set user session */				
				$this->session->set_userdata(array('login' =>  $user_data_object));
	...
	public function logout(){
		...
		$this->session->unset_userdata('latestProduct');
		$this->session->unset_userdata('productCategory');
